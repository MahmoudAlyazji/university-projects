...
for triali=1:size(resdat,2)
 ...
 % new data are the residuals after projecting out the best EEG fit
 resdat(:,triali) = ( EEGdat(:,triali) - yHat )';
end
%% plotting
 % trial averages to see the results clearly. 
figure(1), clf
plot(timevec,mean(eyedat,2), timevec,mean(EEGdat,2),timevec,mean(resdat,2),'linew',2)
legend({'Eye blink';'EEG';'Residual'})
xlabel('Time (ms)')
